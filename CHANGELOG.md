...
- **The `cnd` conformance CLI** (ADR 0020) — `validate`, `hash` and
  ...
  my implementation agree?". It carries only the verbs that *are*
  conformance — `cnd declare` belongs with the producers, since bundling
  it would make the hub depend on a satellite.

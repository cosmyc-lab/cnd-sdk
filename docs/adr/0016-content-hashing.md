...
- A **source hash** — a producer-supplied digest of the *input artifact* it
  built from (a field in the manifest's `source` block). It answers "did the
  input file change" and is only comparable between two manifests from the
  **same producer over the same source**. It is a field decision, covered by
  the 0.3.0 field wave, not by this ADR.
- A **derived content hash** — computed over the *manifest itself*, per node
...
what the manifest necessarily contains is not serialised — ADR 0008 for
edges, ADR 0012 for positions), the hash must be **derived, not a field**.

## Decision
The SDK exposes `node_hash(node)` and `content_hash(manifest)`, and the
...
is: the node `id`, `location`, the resolved `number`, and the manifest's
...
`content_hash(manifest)` folds the node hashes in reading order together with
...
- A normative, cross-language canonicalisation is now part of the conformance
  surface: the golden-fixture corpus must include manifest → expected-hash

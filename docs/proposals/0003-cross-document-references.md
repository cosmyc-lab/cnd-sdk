...
is decided — including whether this belongs in the CND schema at all
...
Consumers that hold a whole corpus of CNDs, not just one, may still
...
1. **`refs_to`/`refs_from` are resolved-pointer contracts today.** Every
   ...
   node — in practice, a node in the *same* CND, since that's the
...
boundary — see Alternatives), would then walk indexed CNDs, build a
...
its own cross-document link table with no CND-level representation
...
retrieval, indexing, or corpus-level concerns, only the CND shape.
If cross-document references end up looking like "a link table keyed by
document id and node id," that arguably needs no new CND field, and
...
it's listed here rather than assumed because a CND-level field would
...
change how `refs_to`/`refs_from` behave for any existing CND.

## Implementation checklist
- [ ] Decide whether this belongs in the CND schema at all, or stays
...
- [ ] Decide who runs resolution and when (indexing time? query time?
      ...
      the CND shape, not on how a consumer resolves it
